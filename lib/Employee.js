  ...
  inquirer
    .prompt({
      ...
      choices: [
        ...
        { value: "35", name: "View Total Budget By Department" },
...
//
// Function to handle viewing total budget by department
//
const selectEmployeeBudgetByDept = () => {
  //
  // Select the employee to update
  //
  conn.query("SELECT name, id FROM departments", (err, depts) => {
    //
    inquirer
      .prompt([
        {
      ...
      .then((answers) => {
        ...
        //
        conn.query(
          `SELECT d.name, FORMAT(SUM(r.salary), 2) as budget, d.name as department FROM roles as r, departments as d WHERE r.dept_id = d.id AND d.id = ${id}`,
          (error, result) => {
            if (error) throw error;
            console.table(result);
            start();
